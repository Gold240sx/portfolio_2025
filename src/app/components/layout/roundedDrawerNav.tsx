...
const RoundedDrawerNavExample = ({ children }: { children: ReactNode }) => {
  return (
    <div className="flex flex-col flex-grow flex-1 relative bg-neutral-950">
      <RoundedDrawerNav
        links={[
          {
            title: 'Product',
        ...
        navBackground=""
        bodyBackground="bg-white flex-1 m-2"
      >
        <div className="flex flex-col items-center justify-center px-12 py-32 flex-grow">
          {children}
        </div>
      </RoundedDrawerNav>
    </div>
...
const RoundedDrawerNav = ({
  ...
  return (
    <div className="h-full flex flex-col">
      <main className="h-full flex-grow flex-1 flex flex-col min-h-screen">
        <nav onMouseLeave={() => setHovered(null)} className={`${navBackground} p-4`}>
          <div className="flex items-start justify-between">
            ...
            <button
              ...
              className="mt-0.5 block text-2xl text-neutral-50 md:hidden cursor-pointer"
            >
              <FiMenu />
            </button>
          </div>
        ...
        <div className={`${navBackground} flex flex-col px-2 pb-2`}></div>
        <div className={`${bodyBackground} rounded-2xl inset-0 flex-grow flex-1 h-auto`}>
          {children}
        </div>
        <Footer />
      </main>
  ...
  return (
    <AnimatePresence mode="popLayout">
      {open && (
        ...
        >
          <div className="grid grid-cols-2 gap-6 py-6 md:hidden">
            {links.map((l) => {
              return (
